...
main = async () => {
    ...
    if (r == 3) {
        try {
            ...
            if (res) { console.log("Tabelas criadas com sucesso") }
        } catch (error) {
            console.log(error)
        }
        try {
            let res = await api.populateTables()
            if (res) { console.log("Dados inseridos com sucesso") }
        } catch (error) {
            console.log(error)
        }
    ...
    if (r == 1) {
        ...
        while (aux != 0) {
            ...
            if (posicao.posicao_x == 4 && posicao.posicao_y == 4 && posicao.nome != "QG da liga") {
                m = askAndReturn('1- Andar para frente\n2- Andar para trás\n3- Andar para cima\n4- Andar para baixo\n5- Ir para próxima região\n')
                regiao = "QG da liga"
            } else if (posicao.posicao_x == 0 && posicao.posicao_y == 0 && posicao.nome != "Arkham city") {
