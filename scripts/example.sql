...
RETURNS TRIGGER AS $$
 ...
 begin
  ...
  SELECT dano INTO dano_inimigo FROM tb_npc_inimigo WHERE tb_npc_inimigo.id = NEW.npc_inimigo_id;

  SELECT qtd_ataque INTO qtd_ataque_personagem FROM tb_personagem WHERE tb_personagem.id = NEW.personagem_id;
...
AFTER INSERT ON -- AKI TA FALTANDO
FOR EACH ROW
